...
import static edu.neu.info6205.finalproj.num09.GA.GA.select;
...
public class TSP_GATest {
    public static Logger log = Logger.getLogger(TSP_GATest.class.getName());
    public static CityNode[] myCity = new CityNode[100];
    ...
    public void test1_CityNodeInit() {
        ...
        if( myCity.length!=TSP_GA.popSize )
            test = false;
        ...
        if( cityHMap.size()!=TSP_GA.popSize )
            test = false;
        //check if city number is legal
        for( int i=0; i<TSP_GA.popSize; i++ ) {
            int cityNum = myCity[i].getCityNum();
            if( cityNum>TSP_GA.popSize || cityNum<0 ) {
                test = false;
                break;
            }
        }
    ...
    public void TestTSP_GA(){
        ...
        ArrayList<Individual> currentIndividuals = new ArrayList<Individual>();
        ...
        */
        ...
        // 1-99round, total iteration number = 100
        for (int k = 1; k < 200; k++) {
