...
/*
    public static ArrayList<Chromosome> crossover(Chromosome ch1, Chromosome ch2) {
        if ( ch1 == null || ch2 == null ) { //check the parent
            return null;
        }
        if (ch1.getGenotype() == null || ch2.getGenotype() == null) { //check the parent's genotype
            return null;
        }
        if (ch1.getGenotype().length != ch2.getGenotype().length) { //check the length of parent's genotype
            return null;
        }
        Chromosome child1 = clone(ch1);
        Chromosome child2 = clone(ch2);
        ...
        for (int i = min; i <= max; i++) {
            sb.append(ch1.getGenotype(i).getGene_x());
            sb.append(ch2.getGenotype(i).getGene_y());
            child1.setGenotype( i, new Gene(sb.toString()) );
            sb.setLength(0);  //clear the buffer

            sb.append(ch2.getGenotype(i).getGene_x());
            sb.append(ch1.getGenotype(i).getGene_y());
